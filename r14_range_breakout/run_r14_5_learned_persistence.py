...
def build_meta_labels(control, feat):
    ...
    control['entry_time']=pd.to_datetime(control.entry_time,utc=True)
    control['signal_time']=control.entry_time-pd.Timedelta(minutes=15)
    for tr in control.itertuples(index=False):
        if tr.exit_reason=='END' or tr.symbol not in feat or tr.signal_time not in feat[tr.symbol].index:
            continue
        side=1 if tr.side=='LONG' else -1
        r=feat[tr.symbol].loc[tr.signal_time]
        sg=signals(r)
        if (side>0 and not sg['range_long']) or (side<0 and not sg['range_short']):
            continue
        z=meta_vector(r,side)
        success=(tr.exit_reason in ('RANGE_TARGET','TREND_LOST') and float(tr.net_return)>0)
        z.update({
            'time':tr.signal_time,'symbol':tr.symbol,'side':side,'month':_month(tr.signal_time),
            'success':int(success),'net_return':float(tr.net_return),'exit_reason':tr.exit_reason
        })
        rows.append(z)
    return pd.DataFrame(rows)
...
def build_walkforward_gate(labels, candidates):
    ...
    for i in range(7,len(months)):
        ...
        diagnostics.append({
            'month':test_m,'status':'OOS','train':len(tr),'cal':len(cal),'candidates':len(te),
