...
VERSION = 'V10-R14.1-RANGE-DEFENSE-CONTINUATION'
    ...
    for t in timeline:
        ...
        if pending_entries:
            ...
            for score,s,side,mode,range_stop in queued:
                ...
                positions[s]={'side':side,'entry':opens[s],'time':t,'notional':notional,'entry_cost':cost,'mode':mode,'origin_mode':mode,'best':opens[s],'stop':range_stop}
                used.add(s)
        ...
        for s,p in list(positions.items()):
            ...
            if side>0 and lows[s]<=p['stop']:
                close(s,t,p['stop'],'RANGE_INVALIDATION_STOP' if p.get('origin_mode')=='RANGE' else 'CATASTROPHIC_STOP'); continue
