...
VERSION = 'V10-R14.6-EVENT-LABELED-ECONOMIC-GATE'
...
def signals(r):
    ...
    if st=='RANGE':
        ...
        out['range_long']=bool(raw_long)
        out['range_short']=bool(raw_short)
...
META_FEATURES = [
    'range_width','eff16','touch_balance','touch_min','containment','center_crosses',
    'range_stability','range_mid_slope_abs','atr_pct','atr_ratio','range_width_change4',
    'recent_containment8','center_crosses8','pressure_against','body_against','vol_z',
    'rsi_extreme','ema_slope_against','adx1h','adx1h_delta','ret4h_against',
...
def meta_vector(r, side):
    if side > 0:
        ...
        rr=(r.range_hi-r.close)/max(r.close-stop,1e-12)
        rsi_ext=(50.0-r.rsi)/20.0
    else:
    ...
    return {
        ...
        'touch_min':float(min(lo,hi)),
        'containment':float(r.containment),
        'center_crosses':float(r.center_crosses),
        'range_stability':float(r.range_stability),
        'range_mid_slope_abs':float(abs(r.range_mid_slope)),
        'atr_pct':float(r.atr_pct),
        'atr_ratio':float(r.atr_ratio),
        'range_width_change4':float(r.range_width_change4),
        'recent_containment8':float(r.recent_containment8),
        'center_crosses8':float(r.center_crosses8),
        'pressure_against':float(-side*r.pressure4),
        'body_against':float(-side*r.body_pressure4),
        'vol_z':float(r.vol_z) if np.isfinite(r.vol_z) else 0.0,
        'rsi_extreme':float(rsi_ext),
        'ema_slope_against':float(-side*r.ema20_slope),
        'adx1h':float(r.adx1h),
        'adx1h_delta':float(r.adx1h_delta) if np.isfinite(r.adx1h_delta) else 0.0,
        'ret4h_against':float(-side*r.ret4h) if np.isfinite(r.ret4h) else 0.0,
        'ret12h_against':float(-side*r.ret12h) if np.isfinite(r.ret12h) else 0.0,
...
def candidate_outcome(d, row, one_way_cost=ONE_WAY_COST):
    t=pd.Timestamp(row.time)
    side=int(row.side)
    entry_t=t+pd.Timedelta(minutes=15)
    if t not in d.index or entry_t not in d.index:
        return None
    sr=d.loc[t]
    er=d.loc[entry_t]
    if side>0:
        range_stop=max(float(sr.range_lo-RANGE_STOP_ATR*sr.atr), float(sr.close*(1-RANGE_STOP_MAX_LOSS)))
        if float(er.open)<=range_stop: return None
    else:
        range_stop=min(float(sr.range_hi+RANGE_STOP_ATR*sr.atr), float(sr.close*(1+RANGE_STOP_MAX_LOSS)))
        if float(er.open)>=range_stop: return None

    entry=float(er.open); best=entry; mode='RANGE'; invalid_count=0; pending=None
    last_time=entry_t; last_close=entry
    for u,r in d.loc[entry_t:END_EXCLUSIVE-pd.Timedelta(minutes=15)].iterrows():
        last_time=u; last_close=float(r.close)
        if pending is not None:
            px=float(r.open)
            gross=side*(px/entry-1.0)
            return {'entry_time':entry_t,'exit_time':u,'net_return':float(gross-2*one_way_cost),
                    'gross_return':float(gross),'exit_reason':pending,
                    'hold_hours':float((u-entry_t)/pd.Timedelta(hours=1))}

        sg=signals(r)
        catastrophic=entry*(1-CATASTROPHIC_STOP if side>0 else 1+CATASTROPHIC_STOP)
        if side>0 and float(r.low)<=catastrophic:
            gross=side*(catastrophic/entry-1.0)
            return {'entry_time':entry_t,'exit_time':u,'net_return':float(gross-2*one_way_cost),
                    'gross_return':float(gross),'exit_reason':'CATASTROPHIC_STOP',
                    'hold_hours':float((u-entry_t)/pd.Timedelta(hours=1))}
        if side<0 and float(r.high)>=catastrophic:
            gross=side*(catastrophic/entry-1.0)
            return {'entry_time':entry_t,'exit_time':u,'net_return':float(gross-2*one_way_cost),
                    'gross_return':float(gross),'exit_reason':'CATASTROPHIC_STOP',
                    'hold_hours':float((u-entry_t)/pd.Timedelta(hours=1))}

        if side>0: best=max(best,float(r.high))
        else: best=min(best,float(r.low))

        if mode=='RANGE':
            inv=(side>0 and float(r.close)<float(r.range_lo-INVALIDATION_ATR*r.atr)) or (side<0 and float(r.close)>float(r.range_hi+INVALIDATION_ATR*r.atr))
            strong_inv=inv and np.isfinite(r.vol_z) and r.vol_z>=BREAKOUT_VOL_Z and ((side>0 and r.ret1<0) or (side<0 and r.ret1>0))
            if inv: invalid_count+=1
            else: invalid_count=max(0,invalid_count-1)
            if strong_inv or invalid_count>=INVALIDATION_CONFIRM_BARS:
                pending='RANGE_INVALIDATION_CONFIRMED'; continue
            if side>0:
                if sg['breakout_up']:
                    mode='TREND'; continue
                if sg['breakout_down']:
                    pending='BREAKOUT_AGAINST'; continue
                if r.range_pos>=RANGE_EXIT_EDGE and (r.close<r.open or r.ret1<0):
                    pending='RANGE_TARGET'; continue
            else:
                if sg['breakout_down']:
                    mode='TREND'; continue
                if sg['breakout_up']:
            ...
            if side>0:
                trail=best*(1-TREND_TRAIL_ATR*max(float(r.atr_pct),0.001))
                if r.close<trail or sg['trend_exit_long'] or sg['breakout_down']:
                    pending='TREND_LOST'
            else:
                trail=best*(1+TREND_TRAIL_ATR*max(float(r.atr_pct),0.001))
                if r.close>trail or sg['trend_exit_short'] or sg['breakout_up']:
                    pending='TREND_LOST'
    gross=side*(last_close/entry-1.0)
    return {'entry_time':entry_t,'exit_time':last_time,'net_return':float(gross-2*one_way_cost),
            'gross_return':float(gross),'exit_reason':'END','hold_hours':float((last_time-entry_t)/pd.Timedelta(hours=1))}


def build_event_labels(candidates, feat):
    rows=[]
    total=len(candidates)
    for i,row in enumerate(candidates.itertuples(index=False),start=1):
        out=candidate_outcome(feat[row.symbol],row,ONE_WAY_COST)
        if out is None or out['exit_reason']=='END':
            continue
        z={k:getattr(row,k) for k in candidates.columns}
        z.update(out)
        z['success']=int(out['net_return']>0)
        rows.append(z)
        if i%500==0: print('LABEL_PROGRESS',i,'/',total,'labels',len(rows),flush=True)
    q=pd.DataFrame(rows)
    if not q.empty:
...
def gate_model():
    return HistGradientBoostingRegressor(
        loss='squared_error',learning_rate=0.04,max_iter=180,max_leaf_nodes=15,
        min_samples_leaf=32,l2_regularization=8.0,random_state=1406
    )
...
def build_walkforward_gate(labels, candidates):
    ...
    for i in range(8,len(months)):
        test_m=months[i]; cal_months=months[i-2:i]; train_months=months[:i-2]
        test_start=pd.Timestamp(test_m+'-01',tz='UTC')
        cal_start=pd.Timestamp(cal_months[0]+'-01',tz='UTC')
        tr=labels[(labels.month.isin(train_months)) & (labels.exit_time<cal_start)].copy()
        cal=labels[(labels.month.isin(cal_months)) & (labels.exit_time<test_start)].copy()
        te=candidates[candidates.month==test_m].copy()
        if len(tr)<900 or len(cal)<80 or te.empty:
            diagnostics.append({'month':test_m,'status':'SKIP','train':len(tr),'cal':len(cal),'candidates':len(te)})
            continue
        model=gate_model()
        y=np.clip(tr.net_return.to_numpy(dtype=float),-0.06,0.08)
        sw=1.0+2.0*np.minimum(np.abs(y)/0.03,1.5)
        model.fit(tr[META_FEATURES],y,sample_weight=sw)
        cal_scores=model.predict(cal[META_FEATURES])
        best=None
        for keep in (0.15,0.20,0.25,0.30,0.35,0.45,0.55):
            thr=float(np.quantile(cal_scores,1.0-keep))
            # Never accept a threshold whose predicted edge is non-positive.
            thr=max(thr,0.0)
            mask=cal_scores>=thr
            n=int(mask.sum())
            if n<max(25,int(0.10*len(cal))): continue
            rets=cal.loc[mask,'net_return'].to_numpy(dtype=float)
            pf=pf_from_returns(rets); mean=float(np.mean(rets)); precision=float((rets>0).mean())
            # A bad calibration regime means NO TRADE next month.
            if mean < 0.001 or pf < 1.10: continue
            objective=mean*math.sqrt(n)*min(pf,2.0)
            ...
            if best is None or cand['objective']>best['objective']:
                best=cand
        if best is None:
            diagnostics.append({'month':test_m,'status':'NO_TRADE_CAL','train':len(tr),'cal':len(cal),'candidates':len(te),
                                'allowed':0,'allowed_share':0.0})
            continue
        test_scores=model.predict(te[META_FEATURES])
        ...
        diagnostics.append({'month':test_m,'status':'OOS','train':len(tr),'cal':len(cal),'candidates':len(te),
...
def simulate(feat, one_way_cost, allowed_keys=None, eval_start=START):
    ...
    timeline=pd.date_range(max(start,eval_start),end-pd.Timedelta(minutes=15),freq='15min',tz='UTC')
    ...
    for t in timeline:
        ...
        for s,p in list(positions.items()):
            ...
            if p['mode']=='RANGE':
                ...
                if side>0:
                    ...
                    if r.range_pos>=RANGE_EXIT_EDGE and (r.close<r.open or r.ret1<0):
                        pending[s]={'reason':'RANGE_TARGET','reverse':'SHORT' if (sg['range_short'] and (allowed_keys is None or (t.value,s,-1) in allowed_keys)) else None}; continue
                else:
                    if sg['breakout_down']:
                        p['mode']='TREND'; continue
                    if sg['breakout_up']:
                        pending[s]={'reason':'BREAKOUT_AGAINST','reverse':None}; continue
                    if r.range_pos<=1-RANGE_EXIT_EDGE and (r.close>r.open or r.ret1>0):
                        pending[s]={'reason':'RANGE_TARGET','reverse':'LONG' if (sg['range_long'] and (allowed_keys is None or (t.value,s,1) in allowed_keys)) else None}; continue
            else: # TREND mode
                if side>0:
        ...
        if slots>0:
            ...
            for s,r in rows.items():
                ...
                sg=sig[s]
                if sg['range_long'] and (allowed_keys is None or (t.value,s,1) in allowed_keys):
                    stop=max(float(r.range_lo-RANGE_STOP_ATR*r.atr), float(r.close*(1-RANGE_STOP_MAX_LOSS)))
                    candidates.append((sg['score_long'],s,1,'RANGE',stop))
                if sg['range_short'] and (allowed_keys is None or (t.value,s,-1) in allowed_keys):
                    stop=min(float(r.range_hi+RANGE_STOP_ATR*r.atr), float(r.close*(1+RANGE_STOP_MAX_LOSS)))
...
def main():
    ...
    if candidates.empty: raise RuntimeError('No raw candidates')
    labels=build_event_labels(candidates,feat)
    print('EVENT_LABELS',len(labels),'RAW_CANDIDATES',len(candidates),flush=True)
    if labels.empty: raise RuntimeError('No event labels')

    allowed,diag=build_walkforward_gate(labels,candidates)
    eligible=diag[diag.status.isin(['OOS','NO_TRADE_CAL'])] if not diag.empty else pd.DataFrame()
    if eligible.empty: raise RuntimeError('No eligible walk-forward months')
    eval_start=pd.Timestamp(str(eligible.month.iloc[0])+'-01',tz='UTC')
    print('EVAL_START',eval_start,'ALLOWED_KEYS',len(allowed),'OOS_MONTHS',int((diag.status=='OOS').sum()),'NO_TRADE_MONTHS',int((diag.status=='NO_TRADE_CAL').sum()),flush=True)
    ...
    ctd,ccd,control_normal=simulate(feat,ONE_WAY_COST,allowed_keys=None,eval_start=eval_start)
    ...
    result={
        ...
        'event_labels':int(len(labels)),'raw_candidates':int(len(candidates)),
        'oos_trade_months':int((diag.status=='OOS').sum()),'no_trade_months':int((diag.status=='NO_TRADE_CAL').sum()),
        'allowed_keys':int(len(allowed)),'normal':normal,'stress':stress,
        ...
        'note':'R14.6 labels every raw range opportunity independently using the unchanged R14.3 lifecycle, then learns expected net return. Training uses only resolved earlier events; two immediately prior months are calibration. If calibration cannot produce positive mean net return and PF >=1.10, the following month is NO TRADE. No minimum holding time and no outer holdout use.'
    }
    Path('r14_6_result.json').write_text(json.dumps(result,indent=2,default=str))
    Path('r14_6_report.md').write_text('# V10 R14.6 Event-Labeled Economic Gate\n\n```json\n'+json.dumps(result,indent=2,default=str)+'\n```\n')
    td.to_csv('r14_6_trades.csv',index=False); std.to_csv('r14_6_trades_stress.csv',index=False)
    ctd.to_csv('r14_6_control_trades.csv',index=False); cstd.to_csv('r14_6_control_trades_stress.csv',index=False)
    labels.to_parquet('r14_6_event_labels.parquet',index=False,compression='zstd')
    candidates.to_parquet('r14_6_candidates.parquet',index=False,compression='zstd')
    diag.to_csv('r14_6_gate_diagnostics.csv',index=False)
    cd.to_parquet('r14_6_equity.parquet',index=False,compression='zstd')
    scd.to_parquet('r14_6_equity_stress.parquet',index=False,compression='zstd')
    print('===R14_6_RESULT_JSON==='); print(json.dumps(result,separators=(',',':'),default=str)); print('===END_R14_6_RESULT_JSON===')
