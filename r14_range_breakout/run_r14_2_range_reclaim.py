...
VERSION = 'V10-R14.2-RANGE-RECLAIM-CONFIRMATION'
...
RANGE_STOP_ATR = 0.60
RANGE_STOP_MAX_LOSS = 0.040
RECLAIM_ATR = 0.20
INVALIDATION_ATR = 0.40
INVALIDATION_CONFIRM_BARS = 2
MIN_REWARD_RISK = 1.25
MIN_REJECTION_VOL_Z = -0.25
...
def signals(r):
    ...
    if st=='RANGE':
        # Enter only after a sweep/reclaim or a micro structure break back inside the range.
        long_reclaim=(r.low<=r.range_lo+RECLAIM_ATR*r.atr and r.close>=r.range_lo+RECLAIM_ATR*r.atr and r.close>r.prev_high)
        short_reclaim=(r.high>=r.range_hi-RECLAIM_ATR*r.atr and r.close<=r.range_hi-RECLAIM_ATR*r.atr and r.close<r.prev_low)
        vol_ok=(not np.isfinite(r.vol_z)) or r.vol_z>=MIN_REJECTION_VOL_Z
        # Do not fade a strong 1h impulse against the intended side.
        long_ctx=(r.ret4>=-1.0*max(r.atr_pct,0.001) and r.ema20_slope>=-0.003)
        short_ctx=(r.ret4<=1.0*max(r.atr_pct,0.001) and r.ema20_slope<=0.003)
        ...
        out['range_long']=bool(r.range_pos<=RANGE_POS_EDGE and bull_reject and long_reclaim and vol_ok and long_ctx and r.rsi<=52 and long_rr>=MIN_REWARD_RISK)
        out['range_short']=bool(r.range_pos>=1-RANGE_POS_EDGE and bear_reject and short_reclaim and vol_ok and short_ctx and r.rsi>=48 and short_rr>=MIN_REWARD_RISK)
    ...
    for t in timeline:
        ...
        for s,p in list(positions.items()):
            ...
            if p['mode']=='RANGE':
                ...
                if p['invalid_count']>=INVALIDATION_CONFIRM_BARS:
