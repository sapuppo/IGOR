#!/usr/bin/env python3
"""R16.4A - August 2026 one-shot holdout check.
...
- only signals inside August
- any position still open at month end is liquidated at final August 4h close
...
OUT=Path("r16_edge_lab/r16_4a_august_holdout")
OUT.mkdir(parents=True,exist_ok=True)

START=int(pd.Timestamp("2026-08-01T00:00:00Z").timestamp()*1000)
END=int(pd.Timestamp("2026-09-01T00:00:00Z").timestamp()*1000)
BASE_COST=.0016
STRESS_COST=.0021
STOP_ATR=2.0
TARGET_ATR=6.0
...
summary={
 "version":"R16.4A",
 "test_type":"one-shot August 2026 calendar holdout",
 "period":"2026-08-01..2026-08-31 UTC",
 "fresh_start":True,
 ...
 "august_holdout_consumed":True,
 "july_holdout_consumed":False,
 "september_holdout_consumed":False,
 "notes":[
   "No parameter was changed after seeing August data.",
   "Only August signals are allowed; open positions are liquidated at the final August 4h close.",
   "No September candle is used for PnL.",
   "This consumes August 2026 as a future tuning holdout."
 ]
}
